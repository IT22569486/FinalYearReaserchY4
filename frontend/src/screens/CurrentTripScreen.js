...
// const normalizeBusLocation = (location) => {
//   if (!location) return location;

//   const latitude = parseCoordinate(location.latitude ?? location.lat);
//   const longitude = parseCoordinate(location.longitude ?? location.lng);

//   return {
//     ...location,
//     ...(latitude !== undefined ? { latitude, lat: latitude } : {}),
//     ...(longitude !== undefined ? { longitude, lng: longitude } : {}),
//   };
// };

// const normalizeBusPayload = (busPayload) => {
//   if (!busPayload) return busPayload;
//   return {
//     ...busPayload,
//     location: normalizeBusLocation(busPayload.location),
//   };
// };
...
//   const [passengeTripId, setPassengerTripId] = useState(null);
//   const tripStartInProgressRef = useRef(false);
//   const [tripStartedMessage, setTripStartedMessage] = useState(null);
//   const [passengerTripStatus, setPassengerTripStatus] = useState('Waiting'); // 'Waiting', 'Trip Started', 'Trip Ended'
...
//     (async () => {
//       // TESTING MODE: Use bus location as passenger location
//       // This allows testing of auto-start and auto-end trip logic
//       // Passenger location will be synchronized with bus location below
      
//       // Original location code (commented out for testing):
...
//         setBus(normalizeBusPayload(busRes.data));
...
//   useEffect(() => {
//     if (bus && bus.location) {
//       setPassengerLocation(bus.location);
//     }
//   }, [bus?.location]);
...
//       }
//     }
//       // Always check auto-start trip logic, even after notification
//       if (passengerOriginStop) {
//         const busToOrigin = getDistanceKm(
//           bus.location.latitude,
//           bus.location.longitude,
//           passengerOriginStop.lat,
//           passengerOriginStop.lng
//         );
//         const passengerToBus = getDistanceKm(
//           passengerLocation?.latitude,
//           passengerLocation?.longitude,
//           bus.location.latitude,
//           bus.location.longitude
//         );
//         // Only start trip if not already started
//         if (
//           passengerLocation &&
//           !passengeTripId &&
//           !tripStartInProgressRef.current &&
//           busToOrigin <= 0.5 &&
//           passengerToBus <= 0.5
//         ) {
//           tripStartInProgressRef.current = true;
...
//               });
//               setPassengerTripId(response?.data?.id || response?.data?._id || null);
//               const newTripId = response?.data?.id || response?.data?._id || null;
//               setPassengerTripStatus('Trip Started');
//               setTripStartedMessage(`Trip Started! From ${userOrigin} to ${userDestination}`);
//               setTimeout(() => {
//                 setTripStartedMessage(null);
//               }, 4000);
//               sendNotification(
//                 'Trip Started',
...
//             } finally {
//               tripStartInProgressRef.current = false;
//             }
//           };
...
//             setPassengerTripStatus('Trip Ended');
//             sendNotification(
//               'Trip Completed',
...
//       return;
//     }
//     // Throttle calls while still allowing regular updates on live location streams.
//     if (nowMs - lastPredictionRunAtRef.current < 4000) {
//       return;
//     }
...
//       {tripStartedMessage && (
//         <View style={styles.tripStartedBanner}>
//           <Text style={styles.tripStartedText}>{tripStartedMessage}</Text>
//         </View>
//       )}
//       <MapViewComponent 
//         buses={[bus]} 
...
//         {tripStartedMessage && (
//           <View style={styles.tripStartedMessageBox}>
//             <Text style={styles.tripStartedMessageText}>{tripStartedMessage}</Text>
//           </View>
//         )}

//         {/* Passenger Trip ID and Status */}
//         <View style={styles.tripStatusRow}>
//           <Text style={styles.tripStatusLabel}>Trip ID:</Text>
//           <Text style={styles.tripStatusValue}>{passengeTripId ? passengeTripId : 'N/A'}</Text>
//         </View>
//         <View style={styles.tripStatusRow}>
//           <Text style={styles.tripStatusLabel}>Trip Status:</Text>
//           <View style={styles.tripStatusBadgeContainer}>
//             <Text style={
//               passengerTripStatus === 'Trip Ended'
//                 ? styles.tripStatusBadgeEnded
//                 : passengerTripStatus === 'Trip Started'
//                   ? styles.tripStatusBadgeStarted
//                   : styles.tripStatusBadgeWaiting
//             }>
//               {passengerTripStatus}
//             </Text>
//           </View>
...
//         <Text style={styles.tripInfoSubText}>Bus Status: {bus.status}</Text>
...
//           onPress={() => navigation.navigate('BusSafety', { busId: busId })}
//         >
//           <Text style={styles.safetyButtonText}>Check Bus Safety</Text>
//         </TouchableOpacity>
...
//     borderRadius: 6,
//   },
//   tripStatusBadgeContainer: {
//     marginLeft: 8,
//   },
//   tripStatusBadgeWaiting: {
//     fontSize: 14,
//     color: '#fff',
...
const CurrentTripScreen = ({ route }) => {
  ...
  const [tripStartedMessage, setTripStartedMessage] = useState(null);
  const [passengerTripStatus, setPassengerTripStatus] = useState('Waiting'); // 'Waiting', 'Trip Started', 'Trip Ended'
  const [isTripInfoCollapsed, setIsTripInfoCollapsed] = useState(true);
  const navigation = useNavigation();
  const { refreshSession } = useSession(); 
  const lastPassedIndexRef = useRef(-1);
  const originNotifiedRef = useRef(false);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <MapViewComponent 
        ...
        passengerDestinationStop={passengerDestinationStop}
      />
      <View style={[styles.tripInfo, isTripInfoCollapsed && styles.tripInfoCollapsed]}>
        <TouchableOpacity
          style={styles.tripInfoHeader}
          onPress={() => setIsTripInfoCollapsed(prev => !prev)}
          activeOpacity={0.8}
        >
          <View>
            <Text style={styles.tripInfoHeaderTitle}>Trip Info</Text>
            <Text style={styles.tripInfoHeaderSubtitle}>Bus {bus.busId} • {passengerTripStatus}</Text>
          </View>
          <Text style={styles.tripInfoHeaderAction}>{isTripInfoCollapsed ? 'Expand' : 'Collapse'}</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={styles.safetyButton}
          onPress={() => navigation.navigate('BusSafety', { busId: bus.busId || busId })}
        >
          <Text style={styles.safetyButtonText}>Check Bus Safety</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  tripInfoHeaderSubtitle: {
    marginTop: 2,
    fontSize: 12,
    color: '#666',
  },
  tripInfoHeaderAction: {
    color: '#007AFF',
    fontWeight: '600',
    fontSize: 13,
  },
