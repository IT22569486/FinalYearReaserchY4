  ...
  const fetchBuses = useCallback(async () => {
    try {
      ...
      const normalizedBuses = busData.map(bus => ({
        ...
        latitude: parseFloat(bus.latitude || bus.location?.latitude || bus.location?.lat),
        longitude: parseFloat(bus.longitude || bus.location?.longitude || bus.location?.lng),
        speed: parseFloat(bus.speed) || 0,
        passenger_count: parseInt(bus.passenger_count || bus.occupancy, 10) || 0,
  ...
  const handleLocationUpdate = useCallback((data) => {
    setBuses(prevBuses => {
      ...
      const updatedBus = {
        ...
        passenger_count: parseInt(data.passenger_count || data.total_passenger_count, 10) || 0,
        status: 'online',
      ...
      if (index >= 0) {
        ...
        return newBuses;
      } else if (!isNaN(updatedBus.latitude) && !isNaN(updatedBus.longitude)) {
