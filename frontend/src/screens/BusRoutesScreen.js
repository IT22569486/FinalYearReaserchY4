...
const BusRoutesScreen = () => {
  ...
  const filteredBuses = useMemo(() => {
    ...
    // Filter by route IMMEDIATELY - don't wait for route details
    if (selectedRoute) {
      buses = buses.filter((bus) => bus.routeId === selectedRoute);
    }
    
    // Only apply direction/origin filtering when we have the route stops data
    // This prevents showing empty results while route details are loading
